...
_SAFE_C_TP_HASH_FUNCS: set[object] | None = None


def _safe_c_tp_hash_funcs() -> set[object]:
...
class UserDefinedObjectVariable(UserDefinedVariable):
    ...
    def is_hashable(self) -> bool:
        # Check the Python type's __hash__ — mirrors the __hash__ is None
        # check in generic_hash's MRO walk, but side-effect-free.
...
class FrozenDataClassVariable(UserDefinedObjectVariable):
    ...
    def hash_impl(self, tx: "InstructionTranslator") -> tuple[int, bool]:
        ...
        # field values.  Use _RawHash to bypass int.__hash__'s modular reduction.
